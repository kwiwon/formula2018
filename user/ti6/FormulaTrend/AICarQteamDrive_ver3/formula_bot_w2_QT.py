...
class ImageProcessor(object):
    ...
    def find_steering_angle_by_color(img, yellow_img, last_steering_angle, hasSign, sign_id, speed, debug = True):
        ...
        tracks       = map(lambda x: len(x[x > 0]), [sb, sg, sr])
        tracks_seen  = filter(lambda y: y > 0, tracks)
        ...
        if detectTrackInverse > 60 and isSingleTrack == False:
        ...
        if isSingleTrack == False :
            if ImageProcessor.detectYellowWall(yellow_img) == True :
                isSingleTrack = True
                maximum_color_idx = np.argmax(tracks, axis=None)
        ...
        if isSingleTrack == True :
            track_pixel_count = np.count_nonzero(track_list[other_track_idx])
            if countdown > 0 :
                countdown -= 1
            else :
                if middle_count > 10 and signDetected == False and track_pixel_count > 1000: 
                    isSingleTrack = False
        ...
        if middle_count > 5 and isSingleTrack == False:
    ...
    def detectInverse(color_px, middle_px, color_id):
        ...
        if maxValue > 50 :
            if math.isnan(color_px) == False:
                leftTrackColor, rightTrackColor = ImageProcessor.getTrackColor()
                if color_px < middle_px :
                    if leftTrackColor != color_id :
                        detectTrackInverse += 1
                    else :
                        if detectTrackInverse > 0 : detectTrackInverse -= 1
                else :
...
class AutoDrive(object):
    ...
    def cross_line(self):
        if len(self._steering_history) != self.MAX_STEERING_HISTORY or self.MAX_STEERING_HISTORY==1:
            return False
        if self._steering_history[-2] > 0 and self._steering_history[-1] < 0:
            self.cross_center+=1
            return True
        elif self._steering_history[-2] < 0 and self._steering_history[-1] > 0:
            self.cross_center-=1
            return True
        else :
            return False
    ...
    def on_dashboard(self, src_img, last_steering_angle, speed, throttle, info):
        ...
        if self.try_hit_wall_recover_times > 0 :
            ...
            self._car.control(ImageProcessor.deg2rad(outAngle), -0.5)
            return
        elif self.detectHitTheWall(speed) :
            detectHitWallTimes = 0
            self.try_hit_wall_recover_times = 7
            #try to recover
            self._car.control(0, -0.5)
            return
        ...
        #detet sign
        if signDetected == True and self.sign_detect_count < self.SIGN_DETECT_DELAY :
        ...
        sign_id = 0
        if len(traffic_sign_imgs) > 0 and signDetected == False and isSingleTrack == False:
            ...
            for i in range(len(traffic_signs)):
                if traffic_signs[i] != 0:
                    ...
                    current_angle = ImageProcessor.find_steering_angle_by_color(track_img, track_yellow_img, last_steering_angle, signDetected, self.sign_detect_id, speed, debug = self.debug)
        else:
            self._traffic_sign_detect_count = [0 for x in range(9)]

            current_angle = ImageProcessor.find_steering_angle_by_color(track_img, track_yellow_img, last_steering_angle, signDetected, self.sign_detect_id, speed, debug = self.debug)

        if try_inverse_recover == 30 : self.try_inverse_recover_angle = current_angle
        if try_inverse_recover > 0:
            try_inverse_recover -= 1
            self._car.control(self.try_inverse_recover_angle, 0.5)
            return
        ...
        if isBack == None :
            ...
            if self.isStatble > self.max_i_len:
                ...
                self.max_i_len = 5;
               # print "=======Stabled========="
            elif self.isStatble < 5 and self.isStatble > 1 :
            ...
            elif angle <95 and angle > 85 and self.started==1:
                self.isStatble = self.isStatble+1
                self.STEERING_PID_Kp = 0.15
                self.STEERING_PID_Ki = 0.00001
                self.STEERING_PID_Kd = 0.015
                ...
                if isSingleTrack :
                    self.THROTTLE_PID_Kp = 0.06
            elif angle <100 and angle > 80 and self.started==1:
                self.isStatble = self.isStatble+1
                self.STEERING_PID_Kp = 0.15
            ...
            elif angle <105 and angle > 75 and self.started==1:
                self.isStatble = self.isStatble+1
                self.STEERING_PID_Kp = 0.15
            ...
            elif angle <120 and angle > 60 and self.started==1:
                self.isStatble = self.isStatble+1
            ...
            elif angle <130 and angle > 50 and self.started==1:
                self.isStatble = self.isStatble+1
            ...
            elif angle <140 and angle > 40 and self.started==1:
                self.isStatble = self.isStatble+1
            ...
            self._car.control(ImageProcessor.deg2rad(outAngle), -0.2)
