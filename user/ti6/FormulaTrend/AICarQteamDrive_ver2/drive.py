...
TRACK_5 = ['UTurnLeft', 'UTurnRight', 'UTurnLeft', 'ForkRight', 'TurnRight', 'UTurnLeft', 'TurnRight']
WEEK_2  = ['TurnLeft', 'ForkRight', 'ForkRight', 'TurnLeft', 'ForkLeft', 'ForkLeft']
TRAFFIC_SIGN_ORDER = WEEK_2
LEN_TRAFFIC_SIGN = len(TRAFFIC_SIGN_ORDER)
...
class twQTeamDrive(object):
    ...
    def __init__(self, car, record_folder = None):
        self.DRIVE_MODE_BAKKUH = 0
        self.DRIVE_MODE_NORMAL = 1

        self.m_drive_mode = self.DRIVE_MODE_NORMAL
        ...
        # self.m_bump_detector = BumpDetector()
        # self.m_bakkuh_driver = Bakkuh()
        # self.m_bakkuh_dir = None

    def clear(self):
        self.m_drive_mode = self.DRIVE_MODE_NORMAL
        # self.m_bump_detector.clear()
    ...
    def normal_drive(self, src_img, last_steering_angle, speed, throttle, info):
        ...
        if len(blob_stats) >= 1:
            if self.m_cnt_order < LEN_TRAFFIC_SIGN:
        ...
        if line_best_idx == -1:
            if self.m_cnt_speed_zero > 15:

                if self.m_is_in_fork_track == False:
                    track_total = list(map(lambda x: len(x[x > 20]), [proc_img[1], proc_img[2]]))
                    max_idx = np.argmax(track_total, axis=None)

                    if max_idx == 0:
                        return 40 * 0.01745, 0.1
                    else:
                        return -40 * 0.01745, 0.1
                else:
                    track_total_black  = list(map(lambda x: len(x[x > 20]), [proc_img[3]]))
                    track_total_yellow = list(map(lambda x: len(x[x > 20]), [proc_img[4]]))
                    if self.m_twQTeamImageProcessor.getTrackMode() == 2:
                        if track_total_black > track_total_yellow:
                            return 40 * 0.01745, 0.1
                        else:
                            return -40 * 0.01745, 0.1
                    elif self.m_twQTeamImageProcessor.getTrackMode() == 3:
                        if track_total_black > track_total_yellow:
                            return -40 * 0.01745, 0.1
                        else:
                            return 40 * 0.01745, 0.1
        ...
        if self.m_is_in_fork_track == False:
            ...
            # if abs(slop_angle) >= 25:
            #     output_throttle = 0.01

            if self.m_cnt_order == 2:
                output_throttle = 0.2
        ...
        if bIsDetectForkSign == False and self.m_is_in_fork_track and self.m_cnt_delay_frame >= 150:
        ...
        return send_steering, send_throttle
        # self.m_car.control(send_steering, send_throttle)

    # def bakkuh_drive(self, rgb_img, speed, last_steering_angle):
    #     bakkuh_status, send_throttle, send_steering = self.m_bakkuh_driver.process(
    #             rgb_img, speed, last_steering_angle, self.m_bakkuh_dir)
    #     if bakkuh_status == Bakkuh.BAKKUH_STATUS_DONE:
    #         self.m_drive_mode = self.DRIVE_MODE_NORMAL
    #     return send_steering, send_throttle
    ...
    def on_dashboard(self, src_img, last_steering_angle, speed, throttle, info):
        ...
        # if self.m_drive_mode == self.DRIVE_MODE_BAKKUH:
        #     send_steering, send_throttle = self.bakkuh_drive(rgb, speed, last_steering_angle)
        #
        # else:
            # bump_status = self.m_bump_detector.detect(rgb, speed, throttle)
            #
            # if bump_status != BumpDetector.NO_BUMP:
            #     self.m_drive_mode = self.DRIVE_MODE_BAKKUH # Switch driving mode
            #     if bump_status == BumpDetector.BUMP_INTO_LEFT_WALL:
            #        self.m_bakkuh_dir = Bakkuh.BAKKUH_DIRECTION_LEFT
            #     elif bump_status == BumpDetector.BUMP_INTO_RIGHT_WALL:
            #         self.m_bakkuh_dir = Bakkuh.BAKKUH_DIRECTION_RIGHT
            #     else:
                    # TODO: bumping into fork or obstacle
                    # self.m_bakkuh_dir = Bakkuh.BAKKUH_DIRECTION_STRIGHT
                # send_steering, send_throttle = 0, 0
            # else:
        send_steering, send_throttle = self.normal_drive(src_img, last_steering_angle, speed, throttle, info)
        self.m_car.control(send_steering, send_throttle)
