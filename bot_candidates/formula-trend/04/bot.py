...
# if want to run it locally, change it to True
RUN_IN_LOCAL = False
if RUN_IN_LOCAL:
    LOGGER_PATH = 'log/'
else:
...
class Log(object):
    def __init__(self, is_debug=True):
        ...
        hdlr = logging.FileHandler(LOGGER_PATH + 'hearts_logs.log')
...
class PID:
    ...
    @staticmethod
    def stability_of_wheel_2(wheel_variants):
        keep_stable = 0
        count = 0
        stability = 0
        for i in range(len(wheel_variants)):
    ...
    def update(self, cur_value1, cur_value, cur_time=None):
        ...
        d_error = error - self._last_error  # differential error
...
class ImageProcessor(object):
    ...
    def turn_to_second_large_color_region(is_left_wall, is_right_wall, tracks):
        turn_index = -1
        if not ImageProcessor.ALREADY_TRANSED:
            if is_left_wall or is_right_wall:
                if not ImageProcessor.is_translating:
    ...
    def find_steering_angle_by_color(img, last_steering_angle, debug=True):
        ...
        px = np.mean(_x)
        # Error handling for
        steering_angle = ImageProcessor.get_color_region_angle(ImageProcessor.current_angle, tracks, track_list, px,
                                                               image_height, camera_x)
...
class AutoDrive(object):
    ...
    def __init__(self, car, car_training_data_collector, record_folder=None):
        ...
        self._steering_pid = PID("wheel", Kp=self.STEERING_PID_Kp, Ki=self.STEERING_PID_Ki, Kd=self.STEERING_PID_Kd,
                                 max_integral=self.STEERING_PID_max_integral)
        self._throttle_pid = PID("throttle", Kp=self.THROTTLE_PID_Kp, Ki=self.THROTTLE_PID_Ki, Kd=self.THROTTLE_PID_Kd,
    ...
    def on_dashboard(self, src_img, last_steering_angle, speed, throttle, info):
        ...
        if self.debug:
            ...
            logit("steering PID: %0.2f (%0.2f) => %0.2f (%0.2f)" % (
                current_angle, ImageProcessor.rad2deg(current_angle), steering_angle,
                ImageProcessor.rad2deg(steering_angle)))
...
if __name__ == "__main__":
    ...
    track_name = "Track_5"
    text_file_path = LOGGER_PATH + "car_training_data_" + track_name + ".txt"
    ...
    def telemetry(sid, dashboard):
        ...
        if dashboard:
            ...
            if (end - begin).total_seconds() > 0.066:
                print('process time too long {} seconds'.format((end - begin).total_seconds()))

            if last_telemetry:
                if (end - last_telemetry).total_seconds() > 0.066:
                    print('telemetry time too long {} seconds'.format((end - last_telemetry).total_seconds()))
